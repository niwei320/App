...
@implementation YFWProgressLayer

- (instancetype)init {
    return [self initWithFrame:CGRectMake(0, 0, PP_SCREEN_WIDTH, 2)];
}
...
- (void)initPathWithWidth:(CGFloat)width {
    ...
    [path addLineToPoint:CGPointMake(PP_SCREEN_WIDTH, width)];
    self.path = path.CGPath;
    self.strokeEnd = 0;
}

- (void)setProgressStyle:(YFWProgressStyle)progressStyle {
    _progressStyle = progressStyle;
    if (progressStyle & YFWProgressStyle_Gradual) {
...
- (void)setProgressColor:(UIColor *)progressColor {
    ...
    if (_progressStyle == YFWProgressStyle_Noraml) {
        self.strokeColor = progressColor.CGColor;
    } else {
        self.progressStyle = YFWProgressStyle_Gradual;
    }
}
...
- (void)progressChanged:(NSTimer *)timer {
    ...
    if (_progressStyle == YFWProgressStyle_Gradual) {
        _gradientLayer.locations = @[@(self.strokeEnd/2), @(self.strokeEnd)];
        _gradientLayer.frame = CGRectMake(0, 0, PP_SCREEN_WIDTH * self.strokeEnd, _lineHeight);
    }
}
