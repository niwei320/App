...
#import "UIDevice+YFWPhoneInfo.h"

#import <sys/utsname.h>
#import "YFWPhoneDefine.h"


@implementation UIDevice (YFWPhoneInfo)
...
+ (BOOL)getDeviceIsIPhone{
    return [UIDevice getDeviceType] == YFWDeviceTypeIPhone;
}
...
+ (BOOL)getDeviceIsIPad{
    return [UIDevice getDeviceType] == YFWDeviceTypeIPad;
}
...
+ (BOOL)getDeviceIsIPod{
    return [UIDevice getDeviceType] == YFWDeviceTypeIPod;
}
...
/**
 * 获取当前设备类型
 * e.g. YFWDeviceTypeIPhone
 @return 当前设备类型的枚举
 */
+ (YFWDeviceType)getDeviceType{
    ...
    if (isSimulator) {
        if (isPhone) {
            return YFWDeviceTypeIPhoneSimulator;
        }else if (isPad){
            return YFWDeviceTypeIPadSimulator;
        }else{
            return YFWDeviceTypeUnkown;
        }
    }
    // 如果是真机
    if (isPhone) {
        return YFWDeviceTypeIPhone;
    }else if (isPad){
        return YFWDeviceTypeIPad;
    }else if (isPod){
        return YFWDeviceTypeIPod;
    }else{
        return YFWDeviceTypeIPadSimulator;
    }
}
