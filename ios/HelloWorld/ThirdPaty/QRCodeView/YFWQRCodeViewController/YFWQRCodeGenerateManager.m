...
@implementation YFWQRCodeGenerateManager
/**
 *  生成一张普通的二维码
 *
...
+ (UIImage *)generateWithLogoQRCodeData:(NSString *)data logoImageName:(NSString *)logoImageName logoScaleToSuperView:(CGFloat)logoScaleToSuperView {
    ...
    CGRect rect = CGRectMake(icon_imageX, icon_imageY, icon_imageW, icon_imageH);
    
    // 根据radius的值画出路线
    
    CGContextAddPath(ctx, [UIBezierPath bezierPathWithRoundedRect:rect cornerRadius:icon_imageW/2].CGPath);
    
    // 裁剪
    
    CGContextClip(ctx);
    
    [icon_image drawInRect:rect];
