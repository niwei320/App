...
-(void)setUpChart{
	if (self.chartStyle == SCChartLineStyle) {
        ...
        if(!_lineChart){
            ...
            _lineChart.showBigPoint = YES;
            _lineChart.showOtherXLabel = YES;
            __weak typeof(self)weakSelf = self;
            _lineChart.selctLineBlock = ^(NSInteger index, CGFloat x_value, CGFloat y_value) {
                if ([weakSelf.dataSource respondsToSelector:@selector(SCChart:ShowIndex:showXValue:showYValue:)]) {
                    [weakSelf.dataSource SCChart:weakSelf ShowIndex:index showXValue:x_value showYValue:y_value];
                }
            };
        ...
        //显示动画
        if ([self.dataSource respondsToSelector:@selector(SCChartShowAnimation:)]) {
            _lineChart.showAnimation = [self.dataSource SCChartShowAnimation:self];
        }
        ...
        [_lineChart setX_new_Labels:[self.dataSource SCChart_new_xLableArray:self]];
