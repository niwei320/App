...
#include <arpa/inet.h>
#include <ifaddrs.h>
#import "YFWKitMacro.h"
#import "NSString+YFWAdd.h"

YFWSYNTH_DUMMY_CLASS(UIDevice_YFWAdd)

@implementation UIDevice (YFWAdd)
...
typedef struct {
    ...
    uint64_t awdl_in;
    uint64_t awdl_out;
} yfw_net_interface_counter;


static uint64_t yfw_net_counter_add(uint64_t counter, uint64_t bytes) {
...
static uint64_t yfw_net_counter_get_by_type(yfw_net_interface_counter *counter, YFWNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & YFWNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & YFWNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & YFWNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & YFWNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & YFWNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & YFWNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static yfw_net_interface_counter yfw_get_net_interface_counter() {
    ...
    yfw_net_interface_counter counter = {0};
        ...
        while (cursor) {
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    counter_in = yfw_net_counter_add(counter_in, data->ifi_ibytes);
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = yfw_net_counter_add(counter_out, data->ifi_obytes);
...
- (uint64_t)getNetworkTrafficBytes:(YFWNetworkTrafficType)types {
    yfw_net_interface_counter counter = yfw_get_net_interface_counter();
    return yfw_net_counter_get_by_type(&counter, types);
}
