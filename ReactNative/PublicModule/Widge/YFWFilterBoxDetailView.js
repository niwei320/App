...
export default class YFWFilterBoxDetailView extends Component {
    ...
    _renderLargeListItem = ({ section: section, row: row }) => {
        if(isNotEmpty(this.state.largeListArray[section]?.items[row])){
            let item = this.state.largeListArray[section]?.items[row]
            let textColor = item.select?(this.isFormWD?'#416dff':'#1fdb9b'):'#333'
            return (
                <View style={{height:45,backgroundColor:'white'}}>
                    <TouchableOpacity activeOpacity={1} style={[BaseStyles.leftCenterView,{flex:1}]}
                        ...
                        {item.select ? <Image style={{width:18,height:11,resizeMode:'contain',marginRight:47,tintColor:this.isFormWD?'#416dff':undefined}}
    ...
    _renderLargeSectionHeader = (section: number) => {
        let item = safeObj(this.state.groupArray[section])
        return (
    ...
    _srorllToIndex(index){
        if(Platform.OS == 'ios') {
            this._sectionList && this._sectionList.scrollToLocation({sectionIndex:index,itemIndex:0,viewOffset:30})
        } else {
