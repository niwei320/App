...
import YFWShopCarCell from '../ShopCar/YFWShopCarCellView'
import {
    backGroundColor,
    darkNomalColor,
    darkTextColor,
...
import YFWPrescribedGoodsTitle, {
    ...
    TYPE_SINGLE,
    TYPE_OTC
} from "./YFWPrescribedGoodsTitle";
import YFWMoneyLabel from './YFWMoneyLabel';
import YFWCheckButtonView from '../PublicModule/Widge/YFWCheckButtonView';


export default class YFWSwipeRow extends SwipeRow {
    ...
    render(){
        if(this.props.type=='package' || this.props.type=='courseOfTreatment'){
            ...
            return(
                <SwipeRow
                    ...
                    <View style={[BaseStyles.leftCenterView,{flex:1}]}>
                        <View accessibilityLabel={this.props.accessibilityLabel} style={styles.checkButton}>
                            <YFWCheckButtonView style={{flex:1}} selectFn={()=>this.props.selectFn && this.props.selectFn()}
            ...
            let isOverdue = parseInt(safe(item.item?.dict_store_medicine_status))<0 || this.props.showType=='oto'?true:false
            return (
                <SwipeRow
                    ref={(c) => this.swipeRow = c}
                    key={item.index}
                    ...
                    <View style={styles.rowItem} key={item.index}>
                        <TouchableOpacity activeOpacity={1} style={{flex:1}} onPress={()=>this._selectGoodsItemMethod()}>
                            <YFWShopCarCell Data={item.item} showType={this.props.showType} changeQuantity={(quantity)=>this._changeItemQuantity(quantity)}
