...
import {
    ...
    TouchableOpacity,
    View,
    DeviceEventEmitter, ImageBackground, Keyboard
} from 'react-native';
...
export default class YFWWDShopCarCellView extends Component {
    ...
    _keyboardDidShow(e) {
        if (e.endCoordinates.height < kScreenHeight / 2) {
            this.state.toastPosition = 0
        } else {
            this.state.toastPosition = e.endCoordinates.screenY - 20
        }
        this.setState({});
      }
      _keyboardDidHide() {
        this.setState({toastPosition: 0});
      }
            ...
            <View style={styles.container}>
                <View style={[BaseStyles.leftCenterView, { flex: 1 }]}>
                    <View style={styles.checkButton}>
                        <YFWWDCheckButtonView style={{ flex: 1 }} selectFn={() => this._selectFn()}
                            isOverdue={isOverdue}
                    ...
                    <View style={styles.view1Style}>
                        <View style={{ marginHorizontal: 10, marginTop: 10, flex: 1 }}>
                            ...
                            <View style={[styles.view2Style, { marginTop: 8, marginBottom: 10 }]}>
                                ...
                                {isOverdue ? null : <View style={styles.operatingBox}>
                                    {this.props.Data.quantity == 1 || this.props.Data.quantity == this.props.Data.purchase_minsum
                                        ? <TouchableOpacity
        ...
        return (
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', flex: 1 }}>
                <View style={{ flex: 1 }}>
                    ...
                    {this.props.Data.period_to ? <Text style={styles.regionStyle} numberOfLines={1}>{'效期  ' + this.props.Data.period_to}</Text> : null}
            ...
            return (
                <View style={{height: 24, alignItems: 'center', justifyContent: 'center'}}>
                    <TouchableOpacity activeOpacity={1}
                        ...
                        { minWidth: 30, height: 30, paddingHorizontal:5,borderColor: separatorColor(), borderLeftWidth: 1, borderRightWidth: 1 }]}
    ...
    _subtractionFn() {
        ...
        let purchase_times = parseInt(this.props.Data.purchase_times)
        if (isNaN(purchase_times) || purchase_times <= 0) {
            purchase_times = 1
        }
        var quantity = Number.parseInt(this.state.quantity) - purchase_times;
        if (quantity < Number.parseInt(this.props.Data.purchase_minsum)) {
            YFWToast('低于起购量',{position:this.state.toastPosition});
            return;
        }
    ...
    _inputChangeQuantity(text) {
        ...
        let quantity = getAddCarCount(text,this.props.Data.quantity,this.props.Data.purchase_minsum,-1,this.props.Data.purchase_times,this.props.Data.reserve)
        this._requestUpdateCount(quantity);
        dismissKeyboard_yfw();
    ...
    _requestUpdateCount(quantity) {
        ...
        paramMap.set('__cmd', 'store.buy.cart.editCart');
