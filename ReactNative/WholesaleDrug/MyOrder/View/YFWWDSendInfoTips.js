...
export default class YFWWDSendInfoTips extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            sendInfoData: [],
    ...
    render() {
        ...
        if (isEmpty(this.state.sendInfoData) || isEmpty(this.state.orderNum) || isEmpty(this.state.sendInfoData.desc)) {
            ...
            if (isNotEmpty(this.state.sendInfoData.button_items) && this.state.sendInfoData.button_items.length > 0) {
                return (
                    <View>
                        {this.state.type == 'detail'?this._renderSplitView():<View />}
                        <View style={{width:width-45,flexDirection:'row',marginLeft:32,alignItems:'center',paddingTop:5,paddingBottom:5}}>
    ...
    _renderButtonsItem(item, index) {
        return (
            <TouchableOpacity activeOpacity={1} onPress={()=>this._buttonItemsClick(index)}>
                <View style={{borderRadius:11,borderColor:'#cccccc',borderWidth:1,paddingBottom:5,paddingTop:5,paddingLeft:13,paddingRight:13,marginLeft:20}}>
