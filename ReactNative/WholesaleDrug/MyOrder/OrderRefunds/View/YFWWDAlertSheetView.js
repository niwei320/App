...
export default class YFWWDAlertSheetView extends Component {
    ...
    showView(sectionArray,callBack) {
        ...
        if (!newSectionArray[0].items) {
            ...
            this.setState({
                ...
                dataArray:newSectionArray[selectIndex].items
            })
        }
                ...
                {this.state.sectionArray.map((item,index) => {
                    return (
                        <TouchableOpacity activeOpacity={1} style={{ flex: 1, justifyContent: 'center',alignItems:'center' }}
                            ...
                            <Text style={{ fontSize: 15,fontWeight:!item.select?'400':'500' }} >{item.title}</Text>
                        </TouchableOpacity>
                    )
                })}
            </View>
    ...
    _renderItem = (item) => {
        ...
        let imageSource = isSelect ? YFWImageConst.Icon_select_blue : require('../../../../../img/check_discheck.png')
        return (
