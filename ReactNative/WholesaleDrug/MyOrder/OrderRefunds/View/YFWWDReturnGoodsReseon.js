...
import YFWWDAlertSheetView from './YFWWDAlertSheetView';
import YFWWDRefundsGoodsReasonItem from './YFWWDRefundsGoodsReasonItem';
import YFWWDUploadVoucher from './YFWWDUploadVoucher';
import YFWWDUpLoadInspectionReport from './YFWWDUpLoadInspectionReport';

export default class YFWWDReturnGoodsReseon extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            selectType: 1,
    ...
    componentDidMount() {
        ...
        paramMap.set('__cmd', 'store.buy.order.getReturnGoodsReasonType');
    ...
    _requestAllReason(fatherReason) {
        let paramMap = new Map();
        let viewModel = new YFWRequestViewModel();
        paramMap.set('__cmd', 'store.buy.order.getReturnGoodsReason as reasonA,store.buy.order.getReturnGoodsReason as reasonB,store.buy.order.getReturnGoodsReason as reasonD');
        ...
        return (
            <View>
                <View style={[styles.content, { marginTop: 17, backgroundColor: 'white' }]}>
                    <TouchableOpacity activeOpacity={1} style={{ justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row', height: 27 }}
                ...
                <View style={{ marginLeft: 28, marginRight: 20 }}>
                    ...
                    <Text style={{ fontSize: 12, color: '#999' }}>{'最多' + toDecimal(this.state.orderTotal) + '元（含运费及包装费' + toDecimal(this.state.packageShippingPrice) + '元）'}</Text>
                </View>
        ...
        if (this.state.is_upload_img == 'true') {
            return (
                <View style={[styles.otherContent, { marginTop: 17 }]}>
                    <YFWWDUploadVoucher tips={this.state.tips} is_upload_img={this.state.is_upload_img} ref='upload_img' />
                </View>
            )
        }
    ...
    _renderUpLoadInspectionReport() {
        if (this.state.is_show_upload_img_report == 'true') {
            return (
                <View style={[styles.otherContent, { marginTop: 17 }]}>
                    <YFWWDUpLoadInspectionReport tips={this.state.report_tips} is_upload_report={this.state.is_show_upload_img_report}
                        ref='upload_report' />
                </View>
            )
        }
    ...
    _renderItem = (item) => {
        return (
            <YFWWDRefundsGoodsReasonItem itemData={item} _stateChanged={this.itemStateChange.bind(this)}
