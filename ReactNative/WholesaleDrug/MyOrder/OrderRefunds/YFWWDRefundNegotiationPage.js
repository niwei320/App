...
export default class YFWWDRefundNegotiationPage extends Component {
    ...
    _requestData() {
        ...
        paramMap.set('__cmd', 'store.buy.order.getNegotiatedDetail');
    ...
    _renderItem(item) {
        ...
        return (
            <View style={style.listItem}>
                ...
                {detailText}
                {hasVoucheImage ? <TouchableOpacity style={{ marginTop: 16 * rpx }}
                    onPress={() => { this._showAlert(voucher_images) }}>
                    <Text style={[style.msg, { color: yfwBlueColor() }]}>查看上传凭证</Text>
                </TouchableOpacity> : <View />}
                {hasReportImage ? <TouchableOpacity style={{ marginTop: 16 * rpx }}
                    onPress={() => { this._showAlert(report_images) }}>
                    <Text style={[style.msg, { color: yfwBlueColor() }]}>查看检验报告</Text>
                </TouchableOpacity> : <View />}
            </View>
        )
    }
