...
import {darkStatusBar, itemAddKey, kScreenWidth, adaptSize, kStyleWholesale} from "../../PublicModule/Util/YFWPublicFunction";
...
import { pushWDNavigation, kRoute_order_detail } from '../YFWWDJumpRouting';
import YFWWDMyRatingModel from './Model/YFWWDMyRatingModel';
import { yfwBlueColor } from '../../Utils/YFWColor';

export default class YFWWDMyRating extends Component {
    ...
    requestMyRatingData() {
        ...
        paramMap.set('__cmd', 'store.whole.order.getEvaluationList');
        paramMap.set('pageSize', 10);
        paramMap.set('pageIndex', this.state.pageIndex);

        viewModel.TCPRequest(paramMap, (res) => {
            let showFoot = 0;
            let dataArray = YFWWDMyRatingModel.getModelArray(res.result.dataList);
        ...
        return (
            <View activeOpacity={1}>
                <View style={{backgroundColor:'white',marginTop:adaptSize(17),marginLeft:adaptSize(13),paddingBottom:adaptSize(20),width:kScreenWidth-adaptSize(26),borderRadius:10,shadowOffset:{width: 0,height:5},shadowColor:'black',shadowOpacity:0.2,elevation:10}}>
                    <TouchableOpacity activeOpacity={1} onPress={this.clickItem.bind(this,item)} hitSlop={{top:20,left:20,bottom:20,right:10}} style={{flexDirection:'row',marginHorizontal:adaptSize(21),marginTop:adaptSize(27),alignItems:'center',justifyContent:'space-between'}}>
                        <Text style={{ color:'rgb(153,153,153)',fontSize: 13}}>订单:
                            <Text style={{ color:yfwBlueColor(),fontSize: 13}}> {this.state.dataArray[item.index].order_no}</Text>
                        </Text>
    ...
    clickItem(item) {
        ...
        pushWDNavigation(navigate, {type: kRoute_order_detail, value:this.state.dataArray[item.index].order_no});
    }
