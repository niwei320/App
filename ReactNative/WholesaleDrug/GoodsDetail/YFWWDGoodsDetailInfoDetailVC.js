...
import YFWWDGoodsDetailInfoDetailViewModel from './Model/YFWWDGoodsDetailInfoDetailViewModel'
import YFWTitleView from '../../PublicModule/Widge/YFWTitleView'
import { pushWDNavigation, kRoute_big_picture, kRoute_html } from '../YFWWDJumpRouting';
...
export default class YFWGoodsDetailInfoDetailVC extends Component {
    ...
    fetchAllDataFromServer(){
        ...
        //商家资质
        paramMap.set('getShopQualification',{'storeid':this.props.data.shop_id})
        ...
        viewModel.TCPRequest(paramMap,(res)=>{
            ...
            let model = YFWWDGoodsDetailInfoDetailViewModel.getModelArray(this.props.data, res['result'])
            this.setState({
    ...
    _renderImageCell(item) {
        ...
        return (
            <TouchableOpacity activeOpacity={1} onPress={() => {this._clickShopImages(item,true)}} style={[styles.contentRadius,{flex:1, padding:model.padding, paddingTop:model.padding-15, alignItems:'center', justifyContent:'center',marginTop:10}]}>
                <Image source={{uri:model.image_url}} style={{width:model.width, height:model.height, resizeMode:'contain'}}/>
            </TouchableOpacity>
        )
    }
    ...
    _renderAptitudeImageCell(item) {
        ...
        return (
            <TouchableOpacity activeOpacity={1} onPress={() => {this._clickShopImages(item,true)}} style={[styles.contentRadius,{width:(kScreenWidth-36)/2, backgroundColor:'#fff',marginBottom:10, marginRight:item.index%2==0 ? 10 : 0, paddingTop:10, alignItems:'center', justifyContent:'center'}]}>
                <Image source={{uri:(safe(model.image_url))}} style={{width:133/375.0*kScreenWidth, height:133/375.0*kScreenWidth, resizeMode:'contain'}}/>
    ...
    _clickShopImages(item,showStatus) {
        if(item.item.navigationImages&&item.item.navigationImages.length>0){
            let {navigate} = this.props.navigation;
            pushWDNavigation(navigate,{type:kRoute_big_picture,value:{imgs:item.item.navigationImages,index:item.index,notShowTip:showStatus}})
        }
    }
