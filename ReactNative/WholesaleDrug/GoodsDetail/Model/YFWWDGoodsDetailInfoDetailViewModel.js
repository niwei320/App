...
export default class YFWWDGoodsDetailInfoDetailViewModel {
    static getModelArray(data, result) {
        let model = new YFWWDGoodsDetailInfoDetailViewModel;
    ...
    _getSafeguardInfo(data, result) {
        ...
        /** 商家资质 */
        if(isNotEmpty(result.getShopQualification)&&isNotEmpty(result.getShopQualification.zz_items)&&result.getShopQualification.zz_items.length>0) {
            let aptitudeImages = []
            let navigationImages = []
            for (let index = 0; index < result.getShopQualification.zz_items.length; index++) {
                const element = result.getShopQualification.zz_items[index];
                navigationImages.push(convertShopImage(element.show_image_suffix,element.image_url))
            }
            for (let index = 0; index < result.getShopQualification.zz_items.length; index++) {
                let element = result.getShopQualification.zz_items[index];
        ...
        /** 商家实景 */
        if(result.getShopQualification&&result.getShopQualification.sj_items&&result.getShopQualification.sj_items.length>0) {
            let aptitudeImages = []
            let navigationImages = []
            for (let index = 0; index < result.getShopQualification.sj_items.length; index++) {
                const element = result.getShopQualification.sj_items[index];
                navigationImages.push(convertShopImage(element.show_image_suffix,element.image_url))
            }
            for (let index = 0; index < result.getShopQualification.sj_items.length; index++) {
                let element = result.getShopQualification.sj_items[index];
                element['key'] = index.toString()
    ...
    _getBaseInfo(data, result) {
        ...
        if(data.img_url&&data.img_url.length>0){
            for (let index = 0; index < data.img_url.length; index++) {
                const element = data.img_url[index];
                let imageModel = {
                    key: index.toString(),
                    cell: 'YFWGoodsDetailsImageCell',
                    image_url: this._convertWatermark(safe(element)),
                    width: kScreenWidth-100,
                    height: kScreenWidth-100,
                    padding:36,
                }
                goodsImages.push(imageModel)
            }
    ...
    _getSaferguardModel(result) {
        ...
        if(safeObj(result.getAPPBannerBottom).link&&safeObj(result.getAPPBannerBottom).link.length > 0){
            items.push(
                {
                    key: 'safe3',
