...
export default class YFWWDAlertCouponCollectionListView extends Component {
    ...
    _renderItem = (item,index) => {
        ...
        let btnTitle = isCanGet?'点击领取':''
        let btnBgSource = isCanGet?require('../../../../img/button_djlq.png'):require('../../../../img/button_ylq.png')
                ...
                <View style={styles.cell}>
                    ...
                    <View style={{flex:1,marginLeft:14*scale}}>
                        <View style = {{marginTop:19,flexDirection:'row',alignItems:'center'}}>
                            <View style={{height:adaptSize(16),borderRadius:adaptSize(8),borderWidth: 1,borderColor:'rgb(84,124,255)',marginRight:adaptSize(5),paddingHorizontal:6,justifyContent:'center'}}>
                                <Text style={{fontSize:12,color:'rgb(84,124,255)',includeFontPadding:false}}>{item.item.type==1?'店铺':'单品'}</Text>
                            </View>
                    ...
                    {isCanGet?<TouchableOpacity activeOpacity={1}  style={{width:76*scale,height:37*scale,right:4*scale,bottom:18*scale,position:'absolute',justifyContent:'flex-end',alignItems:'flex-end'}} onPress={()=>
                         {if(isCanGet){this._getCouponMethod(item.item.id,item.index)}}
                        }>
                                <View style={{alignItems: 'center',justifyContent: 'center',backgroundColor:'rgb(84,124,255)',width:62.5*scale,height:22.2*scale,borderRadius:11.1*scale}}>
                                    <Text style={{color:'white',fontSize:12*scale,includeFontPadding:false}}>{btnTitle}</Text>
                                </View>
                    </TouchableOpacity>:
                    <Image style={{width:62.5*scale,height:22.2*scale,right:7*scale,bottom:18*scale,position:'absolute',tintColor:'rgb(84,124,255)'}} source={btnBgSource}></Image>
                    }
                </View>
            </View>
        );
