...
export default class YFWOrderSettlementModel {
    ...
    setModelData(data){
        ...
        if (isNotEmpty(data) && safeArray(data?.result?.list).length > 0) {
            ...
            data.result.list.forEach((item, index) => {
                ...
                let logistic = [];
                if (isNotEmpty(item.logistcs_list) && safeArray(item.logistcs_list).length > 0){
                    item.logistcs_list.forEach((item, index) => {
                        let showName = item.name
                        if (item.dict_shipping_type == 2 && isNotEmpty(item.lname)) {
                            showName = item.name + '(' + item.lname + ')'
                        }
                        logistic.push({
                            id:item.id,
                            name:showName,
                ...
                let packagelist = [];
                if (isNotEmpty(item.package_list) && safeArray(item.package_list).length > 0){
        ...
        return {
            ...
            inquiry_original_price: isNotEmpty(result.inquiry_yh_price)?result.inquiry_yh_price:0,
            inquiry_real_price: isNotEmpty(result.inquiry_price)?result.inquiry_price:0,
            inquiry_price_explain: isNotEmpty(result.inquiry_price_explain)?result.inquiry_price_explain:'活动期间平台补贴',
            inquiry_desc: isNotEmpty(result.inquiry_desc)?result.inquiry_desc:'一次问诊开方5元，若5种（含）以上单轨药，且由2家（含）以上商家供应，则开具不同的处方',
