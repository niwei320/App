...
import {isNotEmpty, mobClick, tcpImage,isEmpty, iphoneTopMargin, adaptSize, convertImg, checkNotLoginIsHiddenPrice, safeArray, safeObj} from "../PublicModule/Util/YFWPublicFunction";
...
export default class YFWHomeScrollListView extends Component {
    ...
    renderItem() {
        // 数组
        var itemAry = [];
        if (!this.props.Data || safeArray(this.props.Data).length == 0) {
            return itemAry
        }
            ...
            itemAry.push(
                <View key={i} style={[styles.itemStyle,{marginLeft:marginLeft} ]}>
                    <TouchableOpacity activeOpacity={1} style={{flex:1}}  onPress={()=>this.clickItems(dataItem)}>
                        <FastImage style={[styles.imgStyle ]} resizeMode={'contain'} source={{uri:img_url}} resizeMethod={'resize'}/>
                        ...
                        {this.state.noLocationHidePrice?<Text style={{fontSize: 11,color: "#999999",marginRight:18}}>仅做信息展示</Text>:
                            <TouchableOpacity activeOpacity={1} style={{marginRight:15}} onPress={()=>{notLoginIsHiddenPrice&&this.doAfterLogin()}}>
                                {isLogin || !notLoginIsHiddenPrice ? (<View style={{flexDirection:'row',justifyContent:'space-between',marginTop:2}}>
