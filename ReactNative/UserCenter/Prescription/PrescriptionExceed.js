...
import { darkStatusBar, isNotEmpty, kScreenWidth, itemAddKey, isIphoneX, adaptSize,strMapToObj } from "../../PublicModule/Util/YFWPublicFunction";
...
export default class PrescriptionExpire extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataArray: [],
    ...
    render() {
        if (this.state.dataArray.length > 0) {
            ...
            <View style={{ flex:1, alignItems: 'center'}}>
                <YFWEmptyView image={require('../../../img/ic_no_quality.png')} title={ '您没有已过期的处方单'} bgColor={'transparent'} />
            </View>)
        }
    }
            ...
            <TouchableOpacity activeOpacity={1}>
                <View style={{ backgroundColor: 'white', marginLeft: adaptSize(13), width: kScreenWidth - adaptSize(26), marginTop: adaptSize(17), paddingBottom: adaptSize(19), borderRadius: 10, shadowOffset: { width: 0, height: 5 }, shadowColor: 'black', shadowOpacity: 0.2, elevation: 10 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: adaptSize(19), marginHorizontal: adaptSize(21) }}>
                        ...
                        <TouchableOpacity activeOpacity={1} style={[styles.button, { borderColor: '#ccc', backgroundColor: '#ccc', marginRight: 20 }]}>
                            <Text style={{ color: 'white', fontSize: 14 }}>已过期</Text>
                        </TouchableOpacity>
                    </View>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: adaptSize(10), marginHorizontal: adaptSize(21) }}>
                        <View style={{ flexDirection: 'row', flex: 1 }}>
                            <Text style={{ fontSize: 11, color: 'rgb(153,153,153)' }}>开方:</Text>
                            <Text numberOfLines={1} style={{ fontSize: 11, color: 'rgb(153,153,153)', flex: 2 }}> {item.item.start_time}</Text>
                            <Text numberOfLines={1} style={{ fontSize: 11, color: 'rgb(153,153,153)', flex: 3 }}>   {item.item.expire_time} 到期</Text>
                        </View>
                    </View>
                </View>
            </TouchableOpacity>
