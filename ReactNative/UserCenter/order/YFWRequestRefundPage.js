...
import {
    ...
    isNotEmpty,
    iphoneBottomMargin,
    kScreenHeight, kScreenWidth, isIphoneX, deepCopyObj, safeObj
} from "../../PublicModule/Util/YFWPublicFunction";
import YFWRequestViewModel from '../../Utils/YFWRequestViewModel';
const width = Dimensions.get('window').width;
const height = Dimensions.get('window').height;
const {StatusBarManager} = NativeModules;
import {pushNavigation, addSessionCount} from '../../Utils/YFWJumpRouting'
...
export default class YFWRequestRefundPage extends React.Component {
    ...
    _requestReturnGoodsDetail() {
        ...
        viewModel.TCPRequest(paramMap, (res)=> {
            ...
            let descArray = safeObj(res.result.descList)
            this.setState({
    ...
    _commit() {
        ...
        viewModel.TCPRequest(paramMap, (res)=> {
            ...
            addSessionCount('OrderStatusVc','订单操作状态页')
            this.props.navigation.replace('OrderStatusVc',{state:{
    ...
    _renderTips(){
        if (isEmpty(this.state.descArray) || this.state.descArray.length == 0) {
            return null
        }
        return(
            <View style={[styles.list,{backgroundColor: "#f5f5f5", shadowColor:'transparent', elevation:0, paddingHorizontal:16, paddingVertical:13}]}>
                <View style={{flax:1}}>
                    <View style={{flexDirection:'row', alignItems: 'center', marginBottom: 7}}>
                        ...
                        <Text style={{marginLeft:7,fontSize:14,color:'#85a694',fontWeight:'500'}}>温馨提示</Text>
                    </View>
                    {this.state.descArray.map((tipStr,index)=>{
                        return (
                            <View key={index+'s'} style={{flexDirection:'row', minHeight: 15}}>
                                <View style={{height:2, width:2, marginTop:7, marginLeft:3, backgroundColor:'#85a694', borderRadius: 10}}/>
                                <Text style={{fontSize:12,color:darkLightColor(),marginLeft:13}}>{tipStr}</Text>
                            </View>
                        )
                    })}
                </View>
    ...
    _renderSubmitButton(){
        ...
        return (
            <View style={{width:kScreenWidth,justifyContent:'center',alignItems:'center',bottom:BottomMargin,position:'absolute',}}>
                <YFWTouchableOpacity style_title={{height:(kScreenWidth-24)/350*42, width:kScreenWidth-24, fontSize: 17, fontWeight:'bold'}}
                                     title={'提交'}
